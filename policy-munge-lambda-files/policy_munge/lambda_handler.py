...
import aws_caller as aws_caller
...
def lambda_handler(event, context):
    logging.info(f"Starting to read configs")
    config = get_config()
    logging.info(f"The configs read successfully.")

    # logging.info(f'Sleeping to 15mins.')
    # time.sleep(900)
    logging.info(f"Creating cognito client")
    cognito_client = aws_caller.create_cognito_client(
        get_config(ConfigKeys.mgmt_account)
    )
    logging.info(f"The cognito client was created successfully.")
    ...
    for user_name in user_info:
        ...
        if user_name[:-3] in pii_users:
            ...
            a_user_filtered_policy_name_list = [
                p
                for p in user_info[user_name]["policy_names"]
                if RBAC_PII_DB_SUFFIX not in p
            ]
        ...
        if user_info[user_name]["role_name"] in existing_role_list:
            if user_info[user_name]["active"]:
                ...
                statement = create_policy_document_from_template(
                    user_name,
                    get_config(ConfigKeys.s3fs_bucket_arn),
                ...
                dict_of_policy_name_to_munged_policy_objects = (
                    chunk_policies_and_return_dict_of_policy_name_to_json(
                        list_of_policy_objects,
                        user_name,
                ...
                policies_json = json.dumps(
                    list(dict_of_policy_name_to_munged_policy_objects.values())
                )
                if not role_needs_update(
                    user_info[user_name]["role_name"], policies_json
                ):
                    logger.info(
                        f"Role {user_info[user_name]['role_name']} does not need to be updated"
                    )
                    continue

                remove_existing_user_policies(
                    user_info[user_name]["role_name"], all_policy_list
                )

                logging.info(f"Creating munged policy/policies for {user_name}...")
                list_of_policy_arns = (
                    create_policies_from_dict_and_return_list_of_policy_arns(
                        dict_of_policy_name_to_munged_policy_objects
                    )
                )
                ...
                logging.info(
                    f"Attaching munged policy/policies to role for {user_name}..."
                )
                attach_policies_to_role(
                    list_of_policy_arns, user_info[user_name]["role_name"]
                )
                ...
                logging.info(
                    f"User: {user_name} has been removed from UserPool - removing user IAM resources"
                )
                remove_existing_user_policies(
                    user_info[user_name]["role_name"], all_policy_list
                )
...
def ensure_roles(existing_role_list, db_user_info, assume_role_document):
    ...
    for user in db_user_info:
        if (
            db_user_info[user]["role_name"] not in existing_role_list
            and db_user_info[user]["active"]
        ):
            logging.info(f"No role found for {user} - Creating role...")
            created_role = aws_caller.create_role_and_await_consistency(
                db_user_info[user]["role_name"], assume_role_document
            )
...
def role_needs_update(role_name: str, desired_policy_json: str) -> bool:
    ...
    hash_role_tag = next(
        (tag for tag in role_tags if tag["Key"] == "AttachedPoliciesHash"), None
    )
...
def get_policy_info(policy_names, all_policy_list):
    def build_policy_object(policy):
        statement = aws_caller.get_policy_statements(
            policy["Arn"], policy["DefaultVersionId"]
        )
        return {
    ...
    requested_policies = [
        policy for policy in all_policy_list if policy["PolicyName"] in policy_names
    ]
...
def chunk_policies_and_return_dict_of_policy_name_to_json(
    policy_object_list, user_name, role_name
):
    policy_object_list = assign_chunk_number_to_objects(
        policy_object_list, CHARS_IN_EMPTY_IAM_TEMPLATE, CHAR_LIMIT_JSON_POLICY
    )
    total_number_of_chunks = (
        policy_object_list[(len(policy_object_list) - 1)]["chunk_number"] + 1
    )
    dict_of_policy_name_to_munged_policy_objects = {}
    for policy in policy_object_list:
        munged_policy_name = (
            f'emrfs_{user_name}-{policy["chunk_number"] + 1}of{total_number_of_chunks}'
        )
        if munged_policy_name in dict_of_policy_name_to_munged_policy_objects:
            dict_of_policy_name_to_munged_policy_objects[munged_policy_name][
                "Statement"
...
def create_policies_from_dict_and_return_list_of_policy_arns(
    dict_of_policy_name_to_munged_policy_objects,
):
    list_of_policy_arns = []
    for policy in dict_of_policy_name_to_munged_policy_objects:
        prevent_matching_sids(
            dict_of_policy_name_to_munged_policy_objects[policy].get("Statement")
        )
        policy_arn = aws_caller.create_policy_from_json_and_return_arn(
            policy, json.dumps(dict_of_policy_name_to_munged_policy_objects[policy])
        )
...
def tag_role_with_policies(
    policy_list: List[str], role_name: str, additional_tags: Dict[str, str]
):
    tag_object_list = []
    for name in policy_list:
        tag_object_list.append(
            {"policy_name": name, "chars": len(name), "chunk_number": None}
        )
    chunked_tag_object_list = assign_chunk_number_to_objects(
        tag_object_list, CHARS_EMPTY_TAG, CHAR_LIMIT_TAG_VALUE
    )
    tag_keys_to_value_list = {}
    total_number_of_chunks = (
        chunked_tag_object_list[(len(chunked_tag_object_list) - 1)]["chunk_number"] + 1
    )
    ...
    tags = [
        *map(
            lambda key: {"Key": key, "Value": "/".join(tag_keys_to_value_list[key])},
            tag_keys_to_value_list,
        ),
...
def create_policy_document_from_template(user_name, bucket_id, kms_arn):
    with pkg_resources.open_text(
        resources, "s3fs_policy_template.json"
    ) as template_file:
