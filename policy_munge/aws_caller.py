...
def create_cognito_client(mgmt_account_role_arn):
    mgmt_account = sts_client.assume_role(
        RoleArn=mgmt_account_role_arn, RoleSessionName="mgmt_cognito_rds_sync_lambda"
    )
    ...
    while next_token is not None:
        if next_token:
            paginated_pii_users = cognito_client.list_users_in_group(
                UserPoolId=user_pool_id, GroupName=pii_group_name, NextToken=next_token
            )
        else:
            paginated_pii_users = cognito_client.list_users_in_group(
                UserPoolId=user_pool_id,
        ...
        if set(["NextToken", "Next"]).intersection(set(paginated_pii_users)):
            next_token = (
                paginated_pii_users["NextToken"]
                if "NextToken" in paginated_pii_users
                else paginated_pii_users["Next"]
            )
        else:
            next_token = None
...
def get_paginated_results_using_marker(
    aws_api_reponse, list, iam_client_call, field_name=None, client_call_args={}
):
    if field_name == None:
    ...
    if aws_api_reponse["IsTruncated"]:
        ...
        get_paginated_results_using_marker(
            res, list, iam_client_call, field_name, client_call_args
        )
    else:
        return list
...
def create_policy_from_json_and_return_arn(policy_name, json_document):
    policy = iam_client.create_policy(
        Path="/emrfs/", PolicyName=policy_name, PolicyDocument=json_document
    )
...
def remove_policy_being_replaced(policy_arn, role_name):
    ...
    try:
        ...
        if e.response["Error"]["Code"] == "NoSuchEntity":
            logger.info(
                f'Policy: "{os.path.basename(policy_arn)}" not found for role: "{role_name}".'
            )
        else:
            raise e

    # deletes policy versions if exist
    versions_list = iam_client.list_policy_versions(PolicyArn=policy_arn)["Versions"]
...
def wait_for_policy_to_exist(arn):
    ...
    waiter.wait(PolicyArn=arn, WaiterConfig={"Delay": 5, "MaxAttempts": 6})
...
def create_role_and_await_consistency(role_name, assume_role_doc):
    iam_client.create_role(
        Path="/emrfs/", RoleName=role_name, AssumeRolePolicyDocument=assume_role_doc
    )
    waiter = iam_client.get_waiter("role_exists")
    waiter.wait(RoleName=role_name, WaiterConfig={"MaxAttempts": 40})
    return role_name
...
def delete_role_tags(tag_name_list, role_name):
    iam_client.untag_role(RoleName=role_name, TagKeys=tag_name_list)
